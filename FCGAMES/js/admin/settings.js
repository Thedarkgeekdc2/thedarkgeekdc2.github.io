...
game:{questionsPerGame:Math.max(10,Math.min(100,Number($('questionsPerGame').value)||24)),startingLives:Math.max(3,Math.min(12,Number($('startingLives').value)||6)),questionTimeSeconds:Math.max(10,Math.min(300,Number($('questionTimeSeconds').value)||60)),pointsPerCorrectAnswer:Math.max(1,Math.min(1000,Number($('pointsPerCorrectAnswer').value)||10)),enableTimer:$('enableTimer').checked,enableLives:$('enableLives').checked,enableStreak:$('enableStreak').checked,enableSounds:$('enableSounds').checked,showAnswerAfterWrong:$('showAnswerAfterWrong').checked,enableSpecialCards:$('enableSpecialCards').checked},
